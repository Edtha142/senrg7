...
const char* MQTT_USUARIO    = "";          // vacio = sin autenticacion
const char* MQTT_PASSWORD_M = "";

// Identificacion de esta maquina (numero unico, 1, 2, 3, 4...)
const int   MACHINE_ID      = 1;

// Sensor SCT-013-100 con burden de 33 Ohm y GAIN_ONE (+/-4.096V)
//   FACTOR_CAL = SCT_RATING / (BURDEN_OHM * SCT_ISEC_A)
//              = 100 / (33 * 0.05) = 60.606
//   Calibrar con pinza amperimetrica: FACTOR_nuevo = FACTOR * (A_real / A_medido)
const int   N_MUESTRAS      = 100;         // muestras para calculo RMS (~5.8 ciclos a 50Hz)
const float FACTOR_CAL      = 60.6f;       // SCT-013-100 con 33 Ohm burden
const float RUIDO_UMBRAL    = 0.15f;       // lecturas < 0.15A se consideran cero

// Estados (deben coincidir con config.json del servidor)
const float UMBRAL_APAGADA   = 0.5f;
const float UMBRAL_ENCENDIDA = 2.0f;
const float UMBRAL_SOBRECARGA = 20.0f;    // >= sobrecarga; entre ENCENDIDA y SOBRECARGA = BORDANDO
...
void setup() {
  ...
  // GAIN_ONE = +/-4.096V: cubre pico maximo del SCT-013-100 con 33 Ohm
  // V_pico_max = 0.05A * 33 Ohm * sqrt(2) = 2.33V  →  sin saturacion
  ads.setGain(GAIN_ONE);
  ads.setDataRate(RATE_ADS1115_860SPS);     // 860 muestras/segundo
  Serial.println("[OK] ADS1115 GAIN_ONE (+/-4.096V), 860 SPS, modo diferencial AIN0-AIN1");
 ...
 */
String determinarEstado(float corriente) {
  if (corriente >= UMBRAL_SOBRECARGA) return "SOBRECARGA";
